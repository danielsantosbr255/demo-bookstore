...
import { HiOutlineBookOpen, HiOutlineCloudUpload, HiOutlineSearch, HiOutlineShoppingCart } from "react-icons/hi";
  ...
  return (
    <section className="bg-showcase-pattern relative text-white hidden lg:flex p-2 w-full h-10/12 rounded-2xl shadow-lg bg-center bg-cover bg-no-repeat">
      ...
      <div className="relative flex flex-col h-full border-2 border-dashed rounded-2xl w-full z-10 gap-10 text-2xl font-bold items-center justify-center text-center">
        ...
        <Block title="Seu Histórico de Leitura" className="top-1/3 left-5" icon={<LuHistory size={20} />} />
        <Block title="Acesse de onde quiser" className="top-1/5 right-10" icon={<HiOutlineCloudUpload size={20} />} />
        <Block title="Esplore novos títulos" className="bottom-10 left-20" icon={<HiOutlineBookOpen size={20} />} />
        <Block title="Busque por Autores" className="bottom-20 right-10" icon={<HiOutlineSearch size={20} />} />
